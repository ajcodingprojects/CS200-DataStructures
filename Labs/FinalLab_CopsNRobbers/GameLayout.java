...
public class GameLayout implements Serializable {
  ...
  //Variable for basic setup if the user chooses to resume the game
  ...
  public GameLayout(String locationsFile, String connectionsFile) {
    ...
    descriptions.get("Start").setHasRobber(true);
  }
  /*
  ...
  /**
   * method that checks if the robber has been found
   * 
   * @return boolean, true or false if the robber has been found
   */ 
  ...
  /**
   * Method to move cop to a initial from a saved file
   * 
   * @return a location description containg the cops current location 
   */
   ...
   * @returns int of the turn number
   */
  public int getTurn() {
    return turnNumber;
  }
  ...
  /*
   * gets the is from saved variable
   * @returns boolean if saved
   */
  public boolean isFromSave() {
    return fromSave;
  }
  /*
   * Sets the from saved variable 
   */
