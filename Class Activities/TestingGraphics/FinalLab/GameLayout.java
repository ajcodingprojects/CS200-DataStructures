...
//  }
  ...
  public void moveRobber(boolean keepTrackRobber) {
    ...
    Set<String> possibleMoves = connections.get(oldLoc);
    Random r = new Random();
    int moveTo = r.ints(1,0,possibleMoves.size()-1).findFirst().getAsInt();
    Iterator<String> cIterator = allLocationConnections(oldLoc);
    while(moveTo > 0){
     cIterator.next();
     moveTo--;
    }
    if(keepTrackRobber){
      String newLoc = cIterator.next();
